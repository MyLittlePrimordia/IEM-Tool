...
const SafeStorageProxy = (typeof Proxy !== 'undefined') ? new Proxy(SafeStorage, {
    ...
    ownKeys(target) {
        try {
            ...
            // Filter out SafeStorage's own internal property names to avoid
            // polluting Object.keys(localStorage) / for...in enumeration
            const internalProps = new Set(Object.getOwnPropertyNames(SafeStorage));
            Object.getOwnPropertyNames(target).forEach(k => {
                if (!internalProps.has(k)) keys.add(k);
            });
