...
// node reference) plus the window.isMonoMode flag it drives. Extracted verbatim
// from the monolithic inline script (audit #4, second slice).
//
// Zero dependency on any other app module -- confirmed by grep before extracting
// (no reference to PEQDB_Module, App, EQ_Module, FindEngine, IEM_Module,
// Tone_Module, CurveUtils, or showToast anywhere in this block). It only touches
// the Web Audio API and the DOM directly.
//
// NOTE: window.toggleAudioMode (the accessibility mono/stereo toggle button
// handler) was deliberately left in the main script, not moved here -- it calls
// Mascot.update() and touches accessibility-icon DOM elements, so it's UI glue
// that happens to use SharedAudio, not part of the audio engine itself. Keeping
// this slice to code with zero cross-module references is what keeps each step
// of this split low-risk.
//
// Still a plain global (not an ES module) for the same reason as utils.js:
// SharedAudio is referenced by name in EQ_Module, TestLab_Module, and Tone_Module
// elsewhere in the still-monolithic script -- converting all of those to
// import/export is a separate, later, bigger-risk step.
// ==========================================================================
    // ==========================================
// SHARED SYSTEM AUDIO & AUDIO WORKLET ROUTING
// ==========================================
